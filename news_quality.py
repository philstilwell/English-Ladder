...
def review_lesson(client,news,lesson,level,model,review_record=None):
    ...
    payload={'evidence':evidence_text(news),'level':level['cefr'],'minimum_reading_sentences':level['min_sentence_count'],'minimum_vocabulary_items':level['min_vocabulary_count'],'minimum_quiz_items':level['min_quiz_count'],'lesson':lesson}
...
Apply the language policy below to the entire lesson, including every definition, answer option and feedback explanation. Assess vocabulary, register, teaching_language and challenge separately in level_checks. For each, supply passed and a concise reason referencing actual terms or wording from this lesson. Judge contextual meaning and learning value, not word length or rarity alone. Reject both excessive difficulty and material that offers too little learning value for the target level. Essential topic words may recur across levels when their treatment is appropriate.

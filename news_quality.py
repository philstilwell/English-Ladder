...
def review_lesson(client,news,lesson,level,model,review_record=None):
    ...
    reading_evidence_pairs = [
        {'reading_sentence_number': index + 1,
         'learner_reading_sentence': sentence,
         'source_excerpt_for_fact_check_only': excerpts[index] if index < len(excerpts) else None}
        for index, sentence in enumerate(reading)
    ]
    payload={'evidence':evidence_text(news),'level':level['cefr'],'minimum_reading_sentences':level['min_sentence_count'],'minimum_vocabulary_items':level['min_vocabulary_count'],'minimum_quiz_items':level['min_quiz_count'],'reserved_vocabulary':level.get('reserved_vocabulary',[]),'reading_evidence_pairs':reading_evidence_pairs,'lesson':lesson}
...
Identify the fields before reviewing: lesson.news_brief_sentences is the only learner-facing News Brief reading. lesson.sentence_evidence and the top-level evidence are internal source material for fact checking, not additional learner reading. Do not count their sentences, judge their language against the learner level, or treat their wording as the draft reading. reading_evidence_pairs displays the same reading sentences beside their supporting source excerpts; it is an alignment aid, not extra lesson content.
The learner reading may simplify and paraphrase the source while preserving its meaning, timing, agency and qualifications. It does not have to match the source wording exactly. A reading sentence may differ from or happen to match its source excerpt; neither is an error by itself. There are two exact-copy requirements: each sentence_evidence excerpt must occur in the supplied source, and grammar.example_quote must occur in the learner's News Brief. Do not apply either requirement to demand that a simplified reading sentence be an exact source quotation.
...
For each quiz item, try to justify every alternative from the reading or taught grammar before accepting the marked answer. If two options satisfy the question, revise the options or make the question more precise. A preference for one grammatical wording is not proof that another is wrong. Do not infer personality, discipline, motives or likely success from a person’s job, sport, nationality or background without evidence. Distinguish what a named reviewer actually wrote from the lesson writer’s interpretation; do not attribute a paraphrase to a direct quote.
...
Check vocabulary occurrence in lesson.news_brief_sentences, not in sentence_evidence or the source article. A useful word introduced by an accurate paraphrase may be taught even if that word is absent from the original source. Before claiming a term is absent, recheck every learner_reading_sentence in the numbered pairs for that complete word or phrase. If present, assess its contextual meaning and level suitability instead of reporting it missing.
...
Verify the grammar by inspecting the actual words and their order, not by trusting the concept label. Inversion requires an auxiliary before its subject; a coordinated verb phrase such as "nor directly determine prices" is not inversion. A quotation attributed to a source speaker must retain the speaker's exact words; synonymous rewrites must be presented as paraphrases without quotation marks. Check answer feedback for unsupported additions as carefully as the reading.

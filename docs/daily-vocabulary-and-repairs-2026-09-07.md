...
No paid generation or image requests were used for the initial vocabulary migration and repair tests. The subsequent live-run calibration below used the scheduled generation service.
...
The next live review confused the adapted reading with source-only quotations and demanded verbatim news language for beginners. The reviewer now receives explicit reading/source pairs, with each field's role explained. Its reasoning allowance increases from 512 to 8,192 tokens, with a 12,288-token response allowance, so checking a complete lesson is not squeezed into a short review. Simplified reading must preserve the source's meaning and qualifications; it does not have to repeat its wording. Every returned rejection still blocks publication.

Daily runs retain the attempted drafts, selected source material and review failures as seven-day GitHub workflow artifacts. Approved earlier levels are retained even if a later level fails. These diagnostics are saved outside the public site, are not automatically treated as publishable content, and contain no API credentials. They allow a failed draft to be inspected directly instead of diagnosing it only from error messages. Tests and ordinary local runs do not write diagnostics unless a destination is explicitly configured.

The fourth manual generation completed all three levels and created a Gemini illustration. Additional Codex review caught a false Advanced inversion explanation, an altered Intermediate direct quotation and several misleading feedback statements. These were corrected directly in the saved edition, with an independent review of the resulting content. The final counts are Beginner 7/8/7, Intermediate 10/8/9 and Advanced 16/13/11 for reading sentences, vocabulary items and quiz questions. Original Gemini approvals remain in each lesson's review history; the corrected edition records its Codex review separately and does not claim a fresh Gemini approval. The same relevant illustration was visually reviewed and reused without another paid image request. Writer and reviewer instructions now explicitly require checking the actual grammatical structure, preserving direct quotations and avoiding unsupported additions in feedback.

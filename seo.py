...
def profile(path, soup):
    ...
    if relative in PAGES:
        ...
        result.update(kind='grammar', concept=c, parent='grammar-concepts.html', level=c['level'],
                      ...
                      description=f'Practice {c["title"]} in English. {c["goal"]} Includes examples, answer feedback and free PDFs.',
        ...
        result.update(kind='news', parent='archive.html', level=LEVELS[level], lesson=lesson, archive=data,
                      title=f'{lesson["title"]} | {level.title()} English News',
                      description=f'{level.title()} English news practice ({day}): {lesson["title"]}. Read the story, learn key vocabulary and grammar, then check your understanding.',
    ...
    result['description'] = concise(result['description'])
    return result
...
def share_image(path, soup, p):
    ...
    image = soup.select_one('.feature-photo img, .reading-photo img, .daily-lesson img.daily-news-image')
    if image:
...
def add_navigation(soup, path, p):
    for node in soup.select('[data-seo-breadcrumbs], [data-seo-related], .work-breadcrumb, nav.breadcrumb'):
        node.decompose()
    crumbs = breadcrumbs(p)
    ...
    if p['kind'] == 'work':
        ...
        for term,number in [('recently',16),('lately',16),('past perfect',26),('even if',32),('even though',32),('relative clause',12),('causal',9),('cause and effect',9)]:
...
def enhance_page(soup, path, prefix):
    path=Path(path);p=profile(path,soup);url=canonical(p['relative']);image=share_image(path,soup,p)
    ...
    script.string=json.dumps(structured_data(p,soup,image),ensure_ascii=False,separators=(',',':')).replace('<','\\u003c')
...
def write_sitemaps():
    ...
    records={};groups={name:[] for name in ['pages','work','grammar','reading','pdfs']}
    def add(relative,content,group,initial_date=''):
        fingerprint=hashlib.sha256(content).hexdigest();old=previous.get(relative,{})
        modified=old.get('lastmod') if old.get('sha256')==fingerprint else (initial_date if not old and initial_date else date.today().isoformat())
    ...
    for path in published_pages():
        ...
        content=(str(soup.main)+str(soup.title)+str(soup.select_one('meta[name="description"]'))+str(soup.select_one('[data-seo-schema]'))).encode()

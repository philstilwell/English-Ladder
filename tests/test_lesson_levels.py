...
class LessonLevelTests(unittest.TestCase):
    ...
    def test_drafter_and_reviewer_receive_the_same_level_specific_policy(self):
        ...
        for config in update_site.LEVELS:
            ...
            self.assertIn(lesson_levels.STORY_TITLE_GUIDANCE, calls[0]['contents'])
            self.assertIn(lesson_levels.STORY_TITLE_GUIDANCE, update_site.build_prompt(self.archive['source'], config))
            self.assertEqual(lesson_levels.STORY_TITLE_GUIDANCE, update_site.build_response_schema(config)['properties']['title']['description'])
            longer_title = dict(lesson, title='How warmer weather can cause more problems for people who travel')
            self.assertGreater(len(longer_title['title'].split()), 10)
            self.assertEqual([], update_site.validate_lesson_data(longer_title, config))

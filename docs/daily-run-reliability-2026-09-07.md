...
| Push or manual `deploy_only` run | Rebuild and verify existing material without paid generation. Lesson commits use `LESSON_PUBLISH_TOKEN` so Cloudflare receives the branch update. |

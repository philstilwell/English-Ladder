...
Each level keeps a rolling 14-day view; `news/YYYY-MM-DD/LEVEL.html` provides dated lesson addresses and `archive.html` provides search across the last 50 calendar days (today plus the previous 49 fixed-EST dates). Older daily news editions and their public pages, illustrations, and associated translation caches are removed daily. Evergreen stories, grammar, and work courses are unaffected. Every daily run uses the same news story at three reading levels. All pages share an editorial design with warm white surfaces, a white sticky menu, charcoal text, blue brand details, and compact controls. Leveled reading lessons use yellow, green, or blue action buttons to match the selected level. Callouts, vocabulary highlights, definitions, and practice panels share matching pale surfaces and readable dark accents.
...
Story titles aim for **10 words or fewer**. This is a soft editing target, not a word-count rejection rule: a slightly longer title is allowed when an essential name or qualification improves clarity or accuracy. The writer, response schema, and independent reviewer receive the same guidance. Titles omit proficiency labels and padded lesson-analysis subtitles; they are never automatically truncated. Existing title copyedits are recorded separately from the original lesson reviews.

Vocabulary boxes offer English, Japanese, Korean, Simplified Chinese, Spanish, and Brazilian Portuguese definitions. The English terms and grammar labels stay visible. Language buttons use the lesson palette, update inline word-help definitions too, and remember only the chosen language. `vocabulary_translations.py` translates the full contextual definitions and separately reviews them before saving them in `data/vocabulary-translations/`. Page rebuilds and student clicks make no AI calls. Missing, damaged, unreviewed, or outdated translations fall back to English with an explicit message. Daily scheduling retries missing translations for the latest three editions under a cumulative $0.30 daily cap; translation failures cannot block English publication. The manual `translate_recent` input refreshes only missing translations for the latest three editions under that same daily cap, without generating lessons or images. Use it after an editorial correction. The manual `translate_archive` workflow input fills the entire archive and the two evergreen stories using a cumulative $10 initial budget. See the translation directory README for offline coverage and recovery commands.

Everyday English in the US uses that same language preference. A six-button selector replaces How to Study, with English only plus the five explanation languages. It controls all 24 unit sidebars and the ready-to-copy AI prompts; the English practice remains unchanged. `us_life_translations.py` prepares and separately edits the explanations with Gemini, and `us_life_language_ui.py` embeds only reviewed records into the page during offline rebuilding. The manual translation workflow has a cumulative $2 initial budget; normal publishing and student language changes incur no Gemini charges. Original Japanese/Mandarin preferences migrate to the shared setting without replacing a newer preference. Missing translations show an explicit message rather than a different foreign language.

`learning.js` adds Read → Practice → Discuss steps, vocabulary definitions that open when selected, quiz progress, and a completion message. Each lesson opens directly with its story. Share your ideas contains six speaking activities: two story questions followed by level-specific vocabulary, grammar, role-play, and follow-up activities using the lesson’s own words and grammar example. The discussion writing box and its notes message have been removed. Grammar selections remain on the current page. The general browser-saving feature and its saved-word/history controls have been removed; `site.js` never reads or writes those retired learning records. Selecting Finish lesson now saves only a completion flag for that story and level in this browser. Circular yellow, green, and blue checkmarks identify all completed levels for the story in the dated feed and archive, regardless of the level currently open. The Completed button stays active: select it again to mark only that level unfinished; the other levels remain marked. Feed and permanent news URLs share the same marker; evergreen stories use separate keys. Markers survive return visits, refresh across open tabs, and stay on the current page if browser storage is blocked. No answers or notes are included, and markers do not sync between browsers or devices. `continue.html` provides a guided review activity with a complete AI prompt. Work courses retain their separate optional draft-saving controls. No writing is transmitted or automatically graded. Without JavaScript, all reading, vocabulary, questions, and discussion prompts remain visible.
...
3. Gemini creates:
   ...
   - one advanced lesson
   - two story-specific discussion prompts within each lesson, using the same three requests; the offline publisher adds four scripted, level-specific speaking activities to every existing and future lesson
4. The script retrieves the linked BBC article as well as the feed summary, trying another candidate if there is insufficient accessible article text. It validates the structured lesson and requires an exact evidence excerpt for every reading sentence. The writer pairs each reading sentence with a numbered source passage assembled by `lesson_evidence.py`; the program creates the reading and evidence lists together and copies the original source text. Grammar examples are also copied from the reading by sentence index. This avoids mismatched lists and retyped quotations while retaining factual and teaching review. A separate Gemini call checks factual support, question correctness, grammar, repetition, and level suitability. Failure leads to a bounded revision attempt; a failed review is never published. When only vocabulary, grammar, quiz or discussion checks fail and the reading has valid source evidence, the next attempt replaces only the failed sections. Vocabulary repairs select exact reading strings by numeric reference; grammar repairs select an exact reading sentence. Other sections remain locked. The entire merged lesson is then validated and independently reviewed. Reading, evidence and editorial failures instead receive the previous draft plus specific feedback for a complete revision. Each level has a limit of three draft attempts per run and six across compatible recovery runs for that date. Approved levels and unfinished drafts are saved between runs. Temporary service errors retry the same request; an unavailable reviewer leaves the draft awaiting review instead of forcing a rewrite. All three levels must pass before public pages are changed. Daily readings must contain at least **6 complete, distinct sentences for Beginner, 8 for Intermediate, and 10 for Advanced**. These are hard minimums with no ten-sentence ceiling. Only the News Brief counts; titles, overviews, questions, blank entries and repeated sentences do not. The response schema, local validator, separate editorial review, archive writer, rebuild preflight and hosting audit enforce the rule. If any level still fails, no new edition is published; a later run resumes from the saved checkpoint within the daily attempt allowance. Vocabulary must also contain at least **6 distinct items for Beginner, 8 for Intermediate, and 10 for Advanced**, with no upper count limit. Every item needs a term, word class and contextual definition. Only complete entries with whole-word or whole-phrase matches in the reading count. Duplicate terms, blank or malformed fields, and words found only inside other words cannot satisfy the minimum. These checks apply to generation, all archives, rebuilds and the final public-page audit. Quizzes must contain at least **6 questions for Beginner, 8 for Intermediate, and 10 for Advanced**, with no upper count limit. Only complete, distinct multiple-choice questions count: each needs three distinct options, one valid answer key, and three aligned feedback explanations. Blank or malformed items, repeated prompts (including changed numbering), and invalid answer keys fail validation. The independent review also rejects ambiguity, reworded repetition and unsupported answers. Generation retries, archive writes, rebuilds and the hosting audit all enforce these minimums; the audit checks the actual rendered questions, options, feedback and progress totals against the approved lesson data.
5. The validated structured lesson data for all three levels is written to `archive/lessons/YYYY-MM-DD.json`.
6. The rolling level pages show up to 14 editions. The searchable archive retains only the last 50 calendar days. Before each scheduled generation attempt, an independent maintenance job removes older source records, public lesson pages, dated images, and provably associated translation caches; rebuilds feeds and search listings; checks the result; and saves it for publication. Cleanup therefore still happens if lesson generation later fails. Every page refresh repeats this rule, including after a publishing retry fetches newer files from GitHub. Git history retains earlier versions for recovery.
...
### Final-attempt failure email

Only the final scheduled attempt (`15 8 * * *`, 03:15 fixed EST) sends an alert when cleanup, lesson generation, publishing verification, vocabulary translation recovery, or the daily illustration fails. Earlier recovery attempts and ordinary pushes do not send failure emails. The message explains the affected stage, lists failed steps and available error details, and links to the GitHub run. Publishing verification requires the intended release date after a successful scheduled generation, so an older edition cannot silently pass as today’s lesson.

Alerts are sent from and to **philstilwell@gmail.com**, using encrypted Gmail SMTP on port 587. Configure a Gmail **app password** directly as the repository secret `SMTP_PASSWORD` at Settings → Secrets and variables → Actions. Do not use or commit the normal Google account password. Google requires two-step verification for app passwords: [Google setup instructions](https://support.google.com/accounts/answer/185833). No additional email service subscription is used. Missing credentials produce a visible notification-job failure; they do not silently report a successful email.

After adding the secret, manually run this workflow with **test_email** enabled. It sends a clearly labeled setup test and skips generation and publishing. For a failure-report preview inside a workflow context, `python failure_alert.py --dry-run` reads GitHub diagnostics but does not send mail. `--test --dry-run` previews the setup message without network calls (provide `GITHUB_REPOSITORY` and `GITHUB_RUN_ID`). GitHub must start and run the workflow for it to send an alert; a GitHub outage or disabled schedule cannot notify through that same workflow.

To preview cleanup without changing files, run `python lesson_retention.py --json`. Use `python update_site.py --refresh-pages` to apply cleanup and rebuild the site without paid AI calls. Do not publish after using the source-only `lesson_retention.py --apply` command until pages and search files have been rebuilt and audited.
...
The review also tries to justify each incorrect option before approving a quiz, rejects unsupported personality inferences, and checks that B1–B2 readings are not dominated by dense quotations or ornate entertainment-review vocabulary. Concise original paraphrases and useful target words matter more than simply making higher levels longer. These instructions improve the review contract but do not guarantee that a model will judge every lesson correctly.

The existing separate review request must return an explicit decision and a reason for **vocabulary, register, teaching language, and challenge**. A missing, malformed or failed assessment blocks approval even when the reviewer reports an overall pass. Real lesson defects feed into the bounded revision process, while temporary reviewer failures retain the same draft for review. Successful future lessons retain the policy version, target level and four assessments in their archive record. Existing historical lessons are not retroactively marked as passing this new review. Tests use synthetic reviewer responses without paid generation; they verify policy delivery, rejection, revision and retained records, not the model’s ability to judge every real language choice correctly.

Push-triggered runs only rebuild existing content with `--refresh-pages`; they never call Gemini. Both scheduled and manual generation runs use `--skip-existing` to preserve a completed edition. Unfinished editions resume from checked, date-specific GitHub artifacts retained for seven days. Reuse requires unchanged generation policy and a matching approval for the exact source, content, level and vocabulary reservations. See [Daily lesson recovery](docs/daily-run-reliability-2026-09-07.md) for limits and recovery behavior.

Generation, independent review and the daily illustration incur Gemini charges; offline tests, page rebuilds and reuse of completed lessons do not. Retrying a draft or review can add charges, including reasoning tokens. The illustration's published output rate is approximately $0.039 per image plus prompt charges, or about $1.20 for one successful image per day over 30 days. Request and draft limits bound repeated work but are not a billing guarantee. Check [Gemini pricing](https://ai.google.dev/gemini-api/docs/pricing) before estimating a paid run.
...
Cloudflare's existing GitHub integration builds the `main` branch using `npm run deploy:cloudflare`. The deployment wrapper uses the exact Wrangler version in `package-lock.json`, runs JavaScript and site/search checks once, then `cloudflare/build.cjs` copies only public assets into ignored `.cf-site/`. `audit_site.py` already includes the full search audit, so publishing does not run `audit_seo.py` a second time. Preflight has a five-minute limit and logs its elapsed time; the publishing verifier allows fifteen minutes for the build and upload. Failed uploads retry the same prepared files up to three times, with 10- and 30-second waits and a two-minute limit per upload attempt. Failed preflight checks stop before uploading. Every HTML/CSS local reference is checked against the upload. Python sources, project notes, credentials, tests, and dependencies are excluded. All current `.html`, PDF, image, prompt-text, sitemap, and lesson-data addresses are preserved.

The daily publishing job saves checked lessons to GitHub before Cloudflare builds them. Lesson commits use the `LESSON_PUBLISH_TOKEN` repository secret rather than GitHub's built-in workflow token, because external deploy integrations do not reliably receive bot-token pushes. Those commits intentionally omit `[skip ci]` so Cloudflare receives them. A separate publishing job checks out the exact saved commit, waits for the complete deployed file manifest, then verifies the current lesson archive, the three lesson pages, the three level feeds, the homepage/root mapping, and private-file 404 responses. The Cloudflare preview check is the blocking deployment gate; the main-domain check uses the same verifier but treats a Cloudflare 403 to the GitHub runner as a warning, since that is edge protection rather than evidence of stale lesson content. A failed publishing job can be rerun by itself without lesson, translation, image, or page generation. It retains expected/live revisions, file differences, and verification logs as a 14-day workflow artifact. A manual `deploy_only` run rebuilds existing pages and checks publishing without generating paid content; when no files change, it does not restart a failed Cloudflare build. See [Publishing recovery](docs/publishing-recovery.md) for Cloudflare settings and recovery steps.

...
Publication checks keep exact content comparisons for current lesson JSON, then verify the current lesson pages, the three level feeds, the homepage/root mapping, and private-file 404 probes. The full-site byte crawler remains available as `cloudflare/verify.cjs` for deeper hosting audits, but the daily workflow uses `cloudflare/verify-current-lesson.cjs` so routine cron checks do not trip Cloudflare edge protection by crawling hundreds of historical files from a GitHub runner. The Cloudflare preview is the blocking deployment check; if the main public hostname returns a Cloudflare 403 only to the GitHub runner, the workflow records a warning rather than failing a good cron run. GitHub's built-in workflow token is not used for lesson update pushes because Cloudflare did not reliably publish a bot-token commit on September 19, 2026.

...
def panel(key, prompts, *, heading=3):
    ...
    for item in prompts:
        ...
        cards.append(f'''<article class="ai-prompt-card" data-ai-card><div class="ai-prompt-intro"><h{heading}>{e(item['title'])}</h{heading}><p>{e(item['description'])}</p></div><div class="ai-prompt-actions"><details class="ai-prompt-preview"><summary>Read the prompt</summary><pre id="{e(ident)}" class="ai-prompt-text" tabindex="0">{e(item['text'])}</pre></details><button type="button" class="secondary-button ai-copy" data-ai-copy-text="{e(ident)}" aria-label="Copy prompt: {e(item['title'])}" hidden>Copy prompt</button><p class="ai-copy-status" data-ai-copy-status role="status" aria-live="polite"></p></div></article>''')

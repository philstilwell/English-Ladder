...
test('original graphics enlarge and keep keyboard focus without redundant text links',()=>{
 for(const number of [1,5,35,44]){
  ...
  assert.equal(box.querySelector('a'),null);assert.equal(trigger.querySelector('.grammar-image-hint'),null);
  assert.equal(doc.querySelector('.concept-graphic figcaption'),null);
